...
class VGG(nn.Module):
    def __init__(self):
        ...
        self.fex = vgg16(weights = VGG16_Weights.IMAGENET1K_V1)
        self.fex.classifier = nn.Sequential(*list(self.fex.classifier.children())[:-1]) 
        in_features = 4096
...
class SWIN(nn.Module):
    def __init__(self):
        ...
        self.class_num = len(config['constants']['fen_pieces'])
        board_dim = config['constants']['CHESS_BOARD_LEN']
        self.fex = SwinModel.from_pretrained("microsoft/swin-tiny-patch4-window7-224")
        in_features = self.fex.config.hidden_size
        out_features = board_dim*board_dim*self.class_num
        self.dropout = nn.Dropout(p=0.1)
        self.fc = nn.Linear(in_features,out_features)
